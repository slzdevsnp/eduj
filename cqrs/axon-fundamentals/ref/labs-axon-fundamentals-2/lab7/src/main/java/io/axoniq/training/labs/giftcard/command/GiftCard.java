...
@Profile("command")
@Aggregate(snapshotTriggerDefinition = "giftCardSnapshotTriggerDefinition")
public class GiftCard {
    ...
    public GiftCard(IssueCardCommand command) {
        ...
        apply(new CardIssuedEvent(command.getCardId(), command.getAmount(), command.getShopId()));
    }
    ...
    public void handle(RedeemCardCommand command) {
        ...
        if (transactions.stream().map(GiftCardTransaction::getTransactionId)
                        .anyMatch(command.getTransactionId()::equals)) {
