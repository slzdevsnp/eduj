...
public class GiftCardUI extends UI {
    ...
    public GiftCardUI(CommandGateway commandGateway,
                      CardSummaryDataProvider cardSummaryDataProvider,
    ...
    protected void init(VaadinRequest vaadinRequest) {
        HorizontalLayout commandBar = new HorizontalLayout(
                issuePanel(), bulkIssuePanel(), redeemPanel(), reimbursePanel(), bookingPanel()
        );
    ...
    private Panel issuePanel() {
        ...
        Button submit = new Button("Submit", evt -> {
            commandGateway.sendAndWait(new IssueCardCommand(
                    id.getValue(), Integer.parseInt(amount.getValue()), shopId.getValue()
            ));
    ...
    private Panel bulkIssuePanel() {
        ...
        Button submit = new Button("Submit", evt -> {
            for (int i = 0; i < Integer.parseInt(number.getValue()); i++) {
                ...
                commandGateway.sendAndWait(new IssueCardCommand(
                        id, Integer.parseInt(amount.getValue()), shopId.getValue()
                ));
            }
            Notification.show("Success", Notification.Type.HUMANIZED_MESSAGE);
            cardSummaryDataProvider.refreshAll();
    ...
    private Panel bookingPanel() {
        ...
        CheckBox partialPayment = new CheckBox("Partial Payment");
        Button submit = new Button("Submit", evt -> {
            int parsedAmount = Integer.parseInt(amount.getValue());
            transactionTemplate.execute(transactionStatus -> {
                commandGateway.send(new PlaceBookingCommand(
                        UUID.randomUUID().toString(), id.getValue(), parsedAmount, partialPayment.getValue()
                ));
                return null;
            });
            Notification.show("Success", Notification.Type.HUMANIZED_MESSAGE);
        ...
        FormLayout form = new FormLayout(id, amount, partialPayment, submit);
        form.setMargin(true);

        return new Panel("Simulate Booking", form);
    }
    ...
    private VerticalLayout summaryGrid() {
        ...
        return new VerticalLayout(grid, new Button("Refresh", e -> cardSummaryDataProvider.refreshAll()));
    }
}
