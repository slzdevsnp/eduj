package uk.co.monotonic.java_interfaces_and_abstraction.m5.after_simple.csv;
...
class CsvPersistor
{
    ...
    private static final int REVENUE_RECEIVED_COL = 2;
    ...
    List<ClientEngagement> load()
    {
        ...
        try (CSVReader reader = new CSVReader(new FileReader(path)))
        {
            ...
            while (iterator.hasNext())
            {
                ...
                final double revenueReceived = Double.parseDouble(row[REVENUE_RECEIVED_COL]);
                engagements.add(new ClientEngagement(client, hoursWorked, revenueReceived));
            }
            return engagements;
        }
        catch (IOException e)
