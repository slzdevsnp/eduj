...
public class GitHubAPIHttpClient {
    ...
    protected JSONArray getNextIssues(Integer page, Instant since) throws InterruptedException {
        ...
        try {
            ...
            switch (jsonResponse.getStatus()){
                case 200:
                    return jsonResponse.getBody().getArray(); //gets out of function
                case 401:
                ...
                case 403:
                    ...
                    return getNextIssues(page, since); //recursive
                default:
            ...
            e.printStackTrace();
            Thread.sleep(5000L);
            return new JSONArray(); //return empty response
        }
    }
    ...
    protected HttpResponse<JsonNode> getNextIssuesAPI(Integer page, Instant since)
        ...
        // if we have github user, pw  config set  in connector config
        ...
        return unirest.asJson();  //actuall http call  that   returns response as json
    }

    protected String constructUrl(Integer page, Instant since){
        //replace stings with config params
        return String.format(  // %s are replaced by variables
