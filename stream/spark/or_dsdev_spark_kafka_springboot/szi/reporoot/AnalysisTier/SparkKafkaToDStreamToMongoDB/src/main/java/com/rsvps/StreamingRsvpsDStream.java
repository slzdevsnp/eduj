...
public class StreamingRsvpsDStream {
    ...
    private static final Logger log =
            Logger.getLogger(StreamingRsvpsDStream.class.getName());

    private static final String APPLICATION_NAME = "Streaming Rsvps DStream";
    private static final String HADOOP_HOME_DIR_VALUE = "/";
    private static final String RUN_LOCAL_WITH_AVAILABLE_CORES = "local[*]";    
    private static final int BATCH_DURATION_INTERVAL_MS = 5000; // 5 secs batch intval

    private static final Map<String, Object> KAFKA_CONSUMER_PROPERTIES;    
    
    private static final String KAFKA_BROKERS = "localhost:9092";
    private static final String KAFKA_OFFSET_RESET_TYPE = "latest"; // latest  read from latest offset
	...
	//writing in a new collection
    private static final String MONGODB_OUTPUT_URI = "mongodb://localhost/meetupDB.rsvpsguests_zero"; // meetupDB.rsvpsguests
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        //business logic
        // transformations, streaming algorithms, etc  eg.. business logic
        JavaDStream<ConsumerRecord<String, String>> rsvpsWithGuestsStream =
                meetupStream.filter(f -> f.value().contains("\"guests\":0")); // f -> !f.value().contains("\"guests\":0")

        rsvpsWithGuestsStream.foreachRDD((JavaRDD<ConsumerRecord<String, String>> r) -> {

            log.log(Level.INFO,"### about to save to mongo the current record:"+r.toDebugString());
            MongoSpark.save(
                    r.map(
        ...
        ////// additonal logging of offsets for base stream

        // some time later, after outputs have completed
        //ConsumerRecord is a key,value String,String pair
        meetupStream.foreachRDD((JavaRDD<ConsumerRecord<String, String>> meetupRDD) -> {
            //dynamically computes offsets for each batch rdd
...
final class MeetupOffsetCommitCallback implements OffsetCommitCallback {
    ...
    private static final Logger log =
            Logger.getLogger(MeetupOffsetCommitCallback.class.getName());
