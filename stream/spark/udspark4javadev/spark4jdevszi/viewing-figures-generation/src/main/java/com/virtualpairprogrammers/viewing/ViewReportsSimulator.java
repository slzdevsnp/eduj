...
public class ViewReportsSimulator 
{
	public static void main (String[] args) throws InterruptedException, FileNotFoundException
	{
		...
		Scanner scan = new Scanner(new FileReader("src/main/resources/final_viewing_figures"));
		int milliseconds = 0;
		while (scan.hasNextLine())
		{
			...
			while (milliseconds < timestamp)
			{
				...
				if (milliseconds % 24 == 0) {
					Thread.sleep(10); //how frequently to produce messages
				}
			}
	...
	private static Map<Integer, String> courseKeys = Stream.of(new Object[][] {
		...
		.collect(Collectors
				.toMap(it -> (Integer)it[0], it -> (String)it[1]));
}
